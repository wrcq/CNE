...
from cne.algorithms import kmedoids_partition
...
def edge_centers_by_partition(
    graph: nx.Graph,
    partitions: list[set[frozenset]],
) -> list[tuple[float, float]]:
    """Use partition mean edge-center as marker point for each cluster."""
...
def main() -> None:
    ...
    parser.add_argument("--max-iter", type=int, default=50, help="k-medoids max iterations")
    parser.add_argument(
        "--show-edge-labels",
        action="store_true",
    ...
    centers = edge_centers_by_partition(graph, partitions)
